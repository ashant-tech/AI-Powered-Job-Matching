"""
Source 3 - Reporter Jobs Gazette (Ethiopian Job Board)
"""
import requests
...
class Source3:
    def __init__(self):
        self.name = "Reporter Jobs Gazette"
        self.base_url = "https://reporterethiopia.com"
    
    def fetch_jobs(self) -> List[Dict]:
        """Fetch jobs from Reporter Jobs Gazette"""
        jobs = []
        
        try:
            # Reporter Jobs section
            ...
            if response.status_code == 200:
                ...
                # Find job listings
                job_listings = soup.find_all('div', class_='job-item') or soup.find_all('div', class_='job-card') or soup.find_all('article', class_='job-post')
                
                for listing in job_listings:
                    job = self._parse_reporter_job(listing)
                    if job:
                        jobs.append(job)
                        
            else:
                print(f"Reporter Jobs returned status code: {response.status_code}")
                return self._get_sample_jobs()
                
        except Exception as e:
            print(f"Error fetching from Reporter Jobs: {e}")
            return self._get_sample_jobs()
    ...
    def _parse_reporter_job(self, listing) -> Dict:
        """Parse individual job listing from Reporter Jobs"""
        try:
            # Extract job title
            title_elem = listing.find('h3') or listing.find('h2') or listing.find('h4')
    ...
    def _get_sample_jobs(self) -> List[Dict]:
        ...
        return [
            {
                "title": "Journalist",
                "company": "Reporter Ethiopia",
                "description": "Experienced journalist needed for news reporting and investigative journalism...",
                "requirements": "Journalism degree, 3+ years experience, Amharic/English fluency",
                "skills": "journalism,reporting,writing,investigative journalism",
                "location": "Addis Ababa, Ethiopia",
                "salary_min": 18000,
                "salary_max": 30000,
                "job_type": "full-time",
                "source": self.name,
                "source_url": "https://reporterethiopia.com/jobs/789"
            },
            {
                "title": "Editor",
                "company": "Media House Ethiopia",
                "description": "Editor needed for content editing and editorial management...",
                "requirements": "Editorial experience, attention to detail, media background",
                "skills": "editing,content management,media,journalism",
                "location": "Addis Ababa, Ethiopia",
                "salary_min": 15000,
                "salary_max": 25000,
                "job_type": "full-time",
                "source": self.name,
                "source_url": "https://reporterethiopia.com/jobs/790"
            }
        ]
