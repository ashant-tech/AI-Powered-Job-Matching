"""
Source 5 - Srafelagi (Ethiopian Construction & Engineering Jobs)
"""
import requests
...
class Source5:
    def __init__(self):
        self.name = "Srafelagi"
        self.base_url = "https://srafelagi.com"
    
    def fetch_jobs(self) -> List[Dict]:
        """Fetch jobs from Srafelagi (construction/engineering focus)"""
        jobs = []
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for listing in job_listings:
                    job = self._parse_srafelagi_job(listing)
                    if job:
                        jobs.append(job)
                        
            else:
                print(f"Srafelagi returned status code: {response.status_code}")
                return self._get_sample_jobs()
                
        except Exception as e:
            print(f"Error fetching from Srafelagi: {e}")
            return self._get_sample_jobs()
        
        time.sleep(2)
        return jobs
    
    def _parse_srafelagi_job(self, listing) -> Dict:
        """Parse individual job listing from Srafelagi"""
        try:
    ...
    def _extract_ethiopian_skills(self, text: str) -> str:
        """Extract construction/engineering skills"""
        ethiopian_skill_keywords = [
            "civil engineering", "architectural", "construction", "project management",
            "autocad", "revit", "surveying", "structural engineering", "electrical engineering",
            "plumbing", "hvac", "site management", "quality control", "health and safety"
        ]
    ...
    def _get_sample_jobs(self) -> List[Dict]:
        """Return sample construction jobs"""
        return [
            {
                "title": "Civil Engineer",
                "company": "Srafelagi Construction",
                "description": "Civil engineer needed for road construction project...",
                "requirements": "Civil engineering degree, 5+ years experience, AutoCAD proficiency",
                "skills": "civil engineering,autocad,construction,project management",
                "location": "Addis Ababa, Ethiopia",
                "salary_min": 25000,
                "salary_max": 45000,
                "job_type": "full-time",
                "source": self.name,
                "source_url": "https://srafelagi.com/job/555"
            }
        ]
