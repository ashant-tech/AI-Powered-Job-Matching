"""
Source 7 - AddisJobs (Addis Ababa focused job board)
"""
import requests
...
class Source7:
    def __init__(self):
        self.name = "AddisJobs"
        self.base_url = "https://addisjobs.com"
    
    def fetch_jobs(self) -> List[Dict]:
        """Fetch jobs from AddisJobs"""
        jobs = []
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for listing in job_listings:
                    job = self._parse_addis_job(listing)
                    if job:
                        jobs.append(job)
                        
            else:
                print(f"AddisJobs returned status code: {response.status_code}")
                return self._get_sample_jobs()
                
        except Exception as e:
            print(f"Error fetching from AddisJobs: {e}")
            return self._get_sample_jobs()
        
        time.sleep(2)
        return jobs
    
    def _parse_addis_job(self, listing) -> Dict:
        """Parse individual job listing from AddisJobs"""
        try:
            ...
            location = location_elem.get_text(strip=True) if location_elem else "Addis Ababa, Ethiopia"
    ...
    def _get_sample_jobs(self) -> List[Dict]:
        """Return sample Addis Ababa jobs"""
        return [
            {
                "title": "Executive Assistant",
                "company": "AddisJobs",
                "description": "Executive assistant needed for CEO support in Addis Ababa...",
                "requirements": "3+ years experience, excellent English, organizational skills",
                "skills": "administrative,english,organizational skills,communication",
                "location": "Addis Ababa, Ethiopia",
                "salary_min": 15000,
                "salary_max": 25000,
                "job_type": "full-time",
                "source": self.name,
                "source_url": "https://addisjobs.com/job/777"
            }
        ]
